...
HTML5 stipulates that `blockquote` is to be used exclusively for external quotations, which complicates its use for pull quotes—i.e., highlighted quotations from one's own text. Heydon Pickering suggests considering the use of `aside` instead, marked with the `aria-labelledby` attribute for better screen reader accessibility.
